...
NHN Cloudは定期的にDBインスタンスのハイパーバイザソフトウェアをアップデートしてセキュリティと安定性を向上させています。
メンテナンス対象ハイパーバイザで稼働中のDBインスタンスは、マイグレーションを通じてメンテナンスが完了したハイパーバイザに移動する必要があります。

DBインスタンスのマイグレーションはNHN Cloudコンソールから開始できます。
以下のガイドに従ってコンソールのマイグレーション機能を利用してください。
まずメンテナンス対象に指定されたDBインスタンスがあるプロジェクトに移動します。

### 1. メンテナンス対象DBインスタンスを確認します。

名前の横に**マイグレーション**ボタンがあるDBインスタンスがメンテナンス対象インスタンスです。

![planned migration 001](https://static.toastoven.net/prod_rds_mssql/20211109/planned_migration_001.png)

**マイグレーション**ボタンの上にマウスカーソルを合わせると、詳細なメンテナンススケジュールを確認できます。

![planned migration 002](https://static.toastoven.net/prod_rds_mssql/20211109/planned_migration_002.png)

### 2. メンテナンス対象DBインスタンスに接続中のアプリケーションソフトウェアを終了します。

DBに接続しているサービスに影響を与えないように、適切な措置を講じてください。
やむを得ずサービスに影響を与えてしまう場合は、NHN Cloudサポートに連絡してください。適切な措置をご案内します。

### 3. マイグレーションボタンをクリックすると、DBインスタンスマイグレーションの確認ウィンドウが表示されるのでマイグレーションボタンをクリックします。

メンテナンス対象DBインスタンス横の**マイグレーション**ボタンをクリックし、確認ウィンドウが表示されたら**マイグレーション**ボタンをクリックします。

![planned migration 003](https://static.toastoven.net/prod_rds_mssql/20211109/planned_migration_003.png)

### 4. DBインスタンスマイグレーションが終わるまで待機します。

DBインスタンスの状態が変わらない場合は、「更新」をお試しください。

![planned migration 004](https://static.toastoven.net/prod_rds_mssql/20211109/planned_migration_004.png)

DBインスタンスのマイグレーション中は一切操作ができません。
DBインスタンスのマイグレーションが正常に完了しない場合は自動的に管理者に報告され、NHN Cloudから別途連絡します。
